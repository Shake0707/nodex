...
export default function AdminSidebar() {
    ...
    return (
        <aside className="w-60 min-h-screen bg-[#1a1a2e] text-gray-300 flex flex-col p-6 fixed left-0 top-0 z-50 max-md:w-full max-md:min-h-auto max-md:relative max-md:flex-row max-md:p-3 max-md:items-center max-md:overflow-x-auto">
            ...
            <nav className="flex flex-col gap-1 flex-1 max-md:flex-row max-md:gap-1">
                {links.map((link) => (
                    <Link
                        key={link.href}
                        href={link.href}
                        className={`block px-4 py-3 rounded-lg text-sm font-medium transition-all max-md:whitespace-nowrap max-md:px-3 max-md:py-2 max-md:text-xs ${pathname === link.href
            ...
            <button
                className="px-4 py-3 rounded-lg text-sm font-medium text-red-400 text-left transition-all hover:bg-red-500/10 cursor-pointer max-md:whitespace-nowrap max-md:px-3 max-md:py-2 max-md:text-xs"
